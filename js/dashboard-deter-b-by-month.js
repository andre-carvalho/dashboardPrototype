...
*/
	...
	buildCharts:function(dimensions,groups) {
		...
		var dateFormat = localeBR.timeFormat('%B/%Y');
		...
		this.lineDistributionByMonth
			.on('preRender', function(chart) {
				...
				.xAxis(d3.svg.axis()
					...
					.tickFormat(localeBR.timeFormat("%b/%Y"))
		...
		.filterPrinter(function(f) {
			...
			return (dt.getMonth()==f[0][1].getMonth() && dt.getFullYear()==f[0][1].getFullYear())?dateFormat(f[0][1]):dateFormat(dt) + ' - ' + dateFormat(f[0][1]);
		});
